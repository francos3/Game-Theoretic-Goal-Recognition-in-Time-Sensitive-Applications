...
bool PathMatrix::erase_disclosing_paths(int k,int &disclosing_paths){
  ...
  for(int i=0;i<Path_Matrix.size();i++){
	...
	cout<<"\tadding to surviving paths:"<<next_path<<",k:"<<k<<endl;
	surviving_paths.insert(next_path);
      }
      else{
	if(debug){
    ...
    /*if(surviving_paths.size()==0){//No more, we are finished
      cout<<"No more surviving paths, we are finished"<<endl;
      finished=true;
      break;
    }*/
    New_Path_Matrix.push_back(surviving_paths);
  ...
  erase_false_undisclosing_paths(k,finished);
  return finished;
}
...
void PathMatrix::erase_false_undisclosing_paths(int k,bool finished){
//All paths are disclosing if no surviving paths are left, need to annotate the remaining ones
  if(finished){//we are finished,cannot remove any more paths
    cout<<"erase_false_undistclosing_paths,all remaining paths are undisclosing, remaining dests:"<<get_remaining_dests()<<endl;
    finished=true;
    //Add all remaining paths at current k level
    if(k>path_T.size()){
      path_T.resize(k+1);
...
void PathMatrix::populate_cover_matrix(){
  ...
  for(int i=0;i<Path_Matrix.size();i++){
    for (auto next_path : Path_Matrix[i]){
      auto ret=path_ids.emplace(next_path,InitialPaths);
      if(debug)
	cout<<"path_id["<<next_path<<"]:"<<InitialPaths<<endl;
      unordered_map<vector<int>,size_t,VectorHash>::iterator it=ret.first;
      active_columns.emplace(InitialPaths,it);
      active_rows.emplace(InitialPaths++,it);
    }
  }
  ...
  for (auto cover_path : path_ids){
    //cout<<"working on cover_path["<<path_counter++<<"]"<<endl;
...
void PathMatrix::calculate_path_min_lambdas_from_cover_matrix(){
  ...
  for (auto current_path : active_rows){
    ...
    if(min_cal&&removed_paths.size()>0){
	...
	//cout<<"\t\tcurrent_path_id:"<<current_path.second<<",cover_path_id:"<<cover_path.second<<flush<<endl;
	path_min_lambda=min(path_min_lambda,cover_matrix[current_path.second->second][cover_path.second->second]);
	//cout<<"hola1"<<flush<<",path_min_lambda:"<<path_min_lambda<<flush<<",path_L.size:"<<path_L.size()<<endl;
      }
    }
    
    if(path_min_lambda==INT_MAX){//if no update, path_lambda==0
      path_min_lambda=0;
    }
...
void PathMatrix::check_for_improvement_on_lambda_values(){
  ...
  for(iteration=1;iteration<iter_limit;iteration++){
    ...
    if(disconnected_graph){
      cout<<"Lambda_Improv,finished, graph is disconnected"<<endl;
    }
...
void PathMatrix::print_exit_statement(){
  ...
  cout<<",avg_dest_intradist:,"<<avg_dest_intradist<<",avg_dest_orig_dist:,"<<avg_dest_orig_dest<<endl;
