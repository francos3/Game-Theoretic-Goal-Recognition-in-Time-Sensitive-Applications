...
void create_grid_nodes_from_file(){
  ...
  {
    if(str.find("quarterly")){
      cout<<"4-way Connectivity"<<endl;
      connectivity=4;
      continue;
    }
    else if(str.find("octile")){
	 ...
	 int counter=0; 
	  ...
	  for(auto it : Final_paths){
	    ...
	    for(size_t i=0;i<it->length();i++){
	      ...
	      if(i>1){
		if(abs(pos.first-old_pos.first)+abs(pos.second-old_pos.second)>0){
	  ...
	  cout<<",Destination OP lengths:,"<<Optimal_path_dests<<",connectivity:,"<<connectivity<<endl;
	  if(!map_display)
	    exit(0);
