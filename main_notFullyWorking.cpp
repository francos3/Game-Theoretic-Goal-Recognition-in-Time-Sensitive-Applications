...
void create_only_grid_edges_from_file(vector<Link> *grid_edges){
  ...
  for(int x=0;x<max_x;x++){
	...
	if(x-1>-1&&coord_map2.find(A_pos)!=coord_map2.end()){
	    ...
	    //cout<<"\t"<<coord_map2[curr_pos]<<"->"<<coord_map2[X_pos]<<endl;
	    if(!is_dest){
	      if(coord_map2[A_pos]!=start){//no imcoming edges into origin of main graph
		grid_edges->push_back(temp_link);
		main_graph_nodes.insert(coord_map2[curr_pos]);
...
void create_FO_graph(){
  ...
  temp_graph.setv();temp_graph.set_number_vertices( main_graph_nodes.size() );
  ...
  for (auto edge : grid_edges){
    if(FO_nodes.find(edge.u)!=FO_nodes.end()){//it is an outgoing invisible node
      //if(edge.u==1795){
	...
	continue;
      ...
      current_nodes.insert(edge.u);current_nodes.insert(edge.v);
    }
  }
  ...
  for (auto origin : NC_nodes){
    counter++;
    if(counter%10==0){
      auto end_time = std::chrono::high_resolution_clock::now(); std::chrono::duration<double> diff = end_time-start_time; auto current_time=diff.count();
    ...
    temp_dijkstra_alg->improve_distances(&FO_nodes,&FO_edges);
    ...
    for(auto edge : visible_outgoing_edges[origin]){
      current_nodes.erase(edge.u);current_nodes.erase(edge.v);
  ...
  for(auto edge : FO_edges){
    ...
    else if(all_destinations.find(edge.v)!=all_destinations.end()){//incoming destination edge
      //Adding reverse edge for backwards dest_to_dest and boundary graphs
  ...
  main_graph.setv();main_graph.set_number_vertices( main_graph_nodes.size() );
  ...
  for(size_t dest=0;dest<Destinations.size();dest++){
    ...
    current_graph[dest].setv(); current_graph[dest].set_number_vertices( main_graph_nodes.size() );
    cout<<"total_nodes:"<<total_nodes<<",current_graph["<<dest<<"]:,"<<current_graph[dest].get_number_vertices()<<endl;
    Dijkstra_algs.push_back(DijkstraShortestPathAlg(&(current_graph[dest])));
    ...
    current_graph_dest_to_dest[dest].setv(); current_graph_dest_to_dest[dest].set_number_vertices( main_graph_nodes.size() );
    cout<<"total_nodes:"<<total_nodes<<",current_graph_dest_to_dest["<<dest<<"]:,"<<current_graph_dest_to_dest[dest].get_number_vertices()<<endl;
...
void create_grid_edges_from_file(){
  ...
  for(size_t dest=0;dest<Destinations.size();dest++){
    ...
    current_graph[dest].set_number_vertices(boundary_edges_nodes[dest].size());
    ...
    Dijkstra_algs.push_back(DijkstraShortestPathAlg(&(current_graph[dest])));
    //dest_to_dest graphs
    current_graph_dest_to_dest[dest].set_number_vertices( dest_to_dest_nodes[dest].size() );
  ...
  //Second, we include outer links for destinations
	...
	if(!optimize_budget){
	  ...
	  for(size_t dest=0;dest<Destinations.size();dest++){
	    ...
	    cout<<"\nDest["<<Destinations[dest]<<"],Nd|:"<<",Nd|_size:"<<neighourhood_nodes_vect[dest].size()<<endl;
	  ...
	  if(!SaT_map){//Already done in create_grid_nodes_and_edges_from_SaT_file
	    cout<<"getting shortest destination path to all nodes from origin in main graph"<<endl;
	  ...
	  for(auto it : all_destinations){
	    ...
	    cout<<"Destination:,"<<it<<",optimal_path:,"<<path->length()<<",Cost:"<<path->Weight()<<endl;
	    //path->PrintOut(cout);
	 ...
	 int counter=0; 
	  ...
	  for(int i=0;i<max_x;i++){
		...
		if(color_map[i][j][0]==1){//origin node;
		  ...
		  if(size>100000){
		  if(i<max_x-1){
		    if(j<max_y-1&&color_map[i+1][j+1][0]==1){//origin node;
			//cout<<FOREGRN<<"\u25A0\u25A0"<<RESETTEXT;
			cout<<FOREGRN<<"\u25A0"<<RESETTEXT;
			continue;
		    }
		  ...
		  if(i>0){
		    ...
		    else if(j<max_y-1&&color_map[i-1][j+1][0]==1){//origin node;
			//cout<<FOREGRN<<"\u25A0\u25A0"<<RESETTEXT;
			cout<<FOREGRN<<"\u25A0"<<RESETTEXT;
			continue;
		    }
		  ...
		  //DESTINATIONS
		  if(color_map[i][j][1]==1){//destination node
		  ...
		  if(i<max_x-1){
		    if(j<max_y-1&&color_map[i+1][j+1][1]==1){//destination node;
			//cout<<FORERED<<"\u25A0\u25A0"<<RESETTEXT;
			cout<<FORERED<<"\u25A0"<<RESETTEXT;
			continue;
		    }
		  ...
		  if(i>0){
		    ...
		    else if(j<max_y-1&&color_map[i-1][j+1][1]==1){//destination node;
			//cout<<FORERED<<"\u25A0\u25A0"<<RESETTEXT;
			cout<<FORERED<<"\u25A0"<<RESETTEXT;
			continue;
		    }
