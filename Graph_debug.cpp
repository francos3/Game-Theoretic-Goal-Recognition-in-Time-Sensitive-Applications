...
void Graph::add_link( const int& s, const int& d, const double& weight )
{
	...
	//if(d==84){
	//  cout<<"Added link"<< s<<",84,code:"<<get_edge_code(start_vertex_pt, end_vertex_pt)<<",weight:,"<<weight<<endl;
	//}
	if(get_edge_code(start_vertex_pt, end_vertex_pt)==2684){
	  cout<<"Added link"<< s<<","<<d<<",code:"<<get_edge_code(start_vertex_pt, end_vertex_pt)<<",weight:,"<<weight<<endl;
	}
...
long Graph::get_edge_code( const BaseVertex* start_vertex_pt, const BaseVertex* end_vertex_pt ) const
{
	...
	if(long(start_vertex_pt->getID())*long(m_nVertexNum)+long(end_vertex_pt->getID())==2684){
	  cout<<"m_nVertexNum:,"<<m_nVertexNum<<",calculating edge,start_vertex_id:,"<<start_vertex_pt->getID()<<",end_vertex_id:,"<<end_vertex_pt->getID()<<",code:,"<< long(start_vertex_pt->getID())*long(m_nVertexNum)+long(end_vertex_pt->getID())<<endl;
	}
...
long Graph::get_edge_code( const int& s, const int& d){
	BaseVertex* start_vertex_pt = get_vertex(s);
	BaseVertex* end_vertex_pt = get_vertex(d);
	return get_edge_code(s,d);
}
